...
function renderProject(project, i18n, modal) {
  ...
  header.append(createElement("span", "", project.number), createElement("span", "project-status", i18n.t(project.statusKey)));
  const action = createElement("span", "project-card-action", i18n.t("projects.detailsAction"));
  ...
  card.append(header, action, title, description, tags);
...
export async function initProjects(i18n, modal) {
  ...
  const render = () => container.replaceChildren(...projects.map((project) => renderProject(project, i18n, modal)));
